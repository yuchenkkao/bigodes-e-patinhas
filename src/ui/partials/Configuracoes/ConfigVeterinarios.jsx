...
export default function ConfigVeterinarios() {
  const { veterinarios: vets, criar, remover } = useVeterinarios();
  ...
  const handleAdd = async (e) => {
    ...
    await criar({ nome, crmv, especialidade: especialidade || 'Clínica Geral' });
        ...
        <tbody>
          {vets.map(vet => (
            <tr key={vet.id}>
              ...
              <td>
                <button className="btn-delete-config text-mode" onClick={() => remover(vet.id)}>
                  <FaTrashAlt /> Remover
                </button>
              </td>
            </tr>
